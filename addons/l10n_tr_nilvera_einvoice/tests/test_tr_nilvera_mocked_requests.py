...
class TestTRNilveraMockedRequests(TestUBLTRCommon):
    ...
    def test_fetching_einvoices(self, mocked_request):
        with patch.object(self.env.cr, 'commit', autospec=True):
            self.env['account.move']._l10n_tr_nilvera_get_documents(journal_type='purchase')
            self.env['account.move']._l10n_tr_nilvera_get_documents(journal_type='purchase')  # Test that the second time it does not fetch again
